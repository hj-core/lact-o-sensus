//! Foundational consensus types for the Lact-O-Sensus Raft engine.
//!
//! This module defines the monotonic identifiers used for log replication,
//! leader election, and exactly-once semantics. These types are implemented
//! as self-validating NewTypes with checked arithmetic to ensure the
//! structural integrity of the replicated state machine.
...
macro_rules! define_u64_newtype {
    ($name:ident, $doc:expr) => {
        ...
        impl $name {
            /// A constant representing the zero value of this type.
            pub const ZERO: Self = Self(0);

            /// Constructs a new instance from a raw u64 value.
        ...
        impl std::ops::Add<u64> for $name {
            ...
            /// Performs checked addition to prevent overflow.
...
impl std::ops::Sub<u64> for LogIndex {
    ...
    /// Performs checked subtraction for LogIndex to prevent underflow.
    ...
    mod log_index {
        ...
        mod new {
            ...
            mod with_valid_input {
                use super::*;
                #[test]
                fn returns_instance_storing_provided_u64() {
        ...
        mod conversions {
            ...
            mod with_u64 {
                use super::*;
                #[test]
                fn supports_lossless_roundtrip() {
                    let raw = 42u64;
        ...
        mod arithmetic {
            ...
            mod addition {
                ...
                #[test]
                fn returns_success_when_sum_is_within_bounds() {
                ...
                #[test]
                fn returns_overflow_error_when_sum_exceeds_u64_max() {
            ...
            mod subtraction {
                ...
                #[test]
                fn returns_success_when_difference_is_positive() {
                    let idx = LogIndex::new(10);
                ...
                #[test]
                fn returns_underflow_error_when_difference_is_negative() {
