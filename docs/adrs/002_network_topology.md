...
- **Zone A (Internal Mesh):** All Raft Cluster Participants (Leader, Followers, and Candidates) maintain full-mesh connectivity for heartbeats, elections, and log replication.
...
- The node should provide a **`leader_hint`**—a logical redirection token containing the metadata required for the Client to re-route its request to the current Leader.
